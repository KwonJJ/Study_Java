...
public class 다차원배열03 {
	...
	public static void main(String[] args) {
		...
		int[][] n = new int[3][2]; // 3줄 2칸짜리 배열 생성
		double[] result = new double[3]; // 평균값 저장하기 위한 배열 생성
		...
		for (int i = 0; i < n.length; i++) { // 배열의 줄 수 조건
			...
			result[i] = (double) sum / 2; // 각 평균값 구한 후 result배열에 삽입
		}
		for (int i = 0; i < 3; i++) { // 3명의 평균값 출력
